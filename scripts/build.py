...
function initializeFilters() {
    const taxonomyFilters = {'filterKingdom':'kingdom','filterPhylum':'phylum_division','filterClass':'class','filterOrder':'order','filterFamily':'family'};
    for (const [selectId, column] of Object.entries(taxonomyFilters)) {
...
function loadFiltersFromUrl() {
    ...
    if(params.get('kingdom'))document.getElementById('filterKingdom').value=params.get('kingdom');
    if(params.get('phylum'))document.getElementById('filterPhylum').value=params.get('phylum');
    if(params.get('class'))document.getElementById('filterClass').value=params.get('class');
    if(params.get('order'))document.getElementById('filterOrder').value=params.get('order');
...
$(document).ready(function() {
    ...
    document.getElementById('resetFilters').addEventListener('click',()=>{document.querySelectorAll('select').forEach(s=>s.value='');document.querySelectorAll('.status-btn').forEach(btn=>btn.classList.add('active'));document.querySelectorAll('.habitat-filter').forEach(cb=>cb.checked=false);document.getElementById('filterProtected').checked=false;applyFilters();});
...
def main():
    ...
    generate_index_html(genes_data, zenodo_links, output_dir, build_date)
...
def generate_report_html(gene_name, display_name, df, stats, jncc_columns, filter_options, output_dir, build_date):
    """Generate an interactive report page for a single gene with lazy data loading."""
...
<head>
    ...
    <style>
        ...
        .filter-panel h5 { margin-bottom:1rem;padding-bottom:.5rem;border-bottom:1px solid #e9ecef; }
        .status-badge { display:inline-block;padding:.25em .6em;font-size:.8rem;font-weight:600;border-radius:4px;color:#fff; }
        .status-GREEN { background:var(--ukbol-green); }
        .status-AMBER { background:var(--ukbol-amber);color:#000; }
        .status-RED { background:var(--ukbol-red); }
        .status-BLUE { background:var(--ukbol-blue); }
        .status-BLACK { background:var(--ukbol-black); }
        .filter-tag { display:inline-flex;align-items:center;gap:.25rem;background:#e7f1ff;color:#0d6efd;padding:.25rem .75rem;border-radius:20px;font-size:.85rem;margin:.25rem; }
        .filter-tag button { background:none;border:none;color:inherit;padding:0;font-size:1rem;cursor:pointer; }
        #activeFilters:empty::before { content:"No filters active";color:#6c757d;font-style:italic; }
        .loading { text-align:center;padding:3rem; }
...
<body>
    <nav class="navbar navbar-dark mb-4">
        <div class="container-fluid">
            <a class="navbar-brand" href="index.html">Back to Portal</a>
            <span class="navbar-text fw-bold">''' + display_name + '''</span>
            <span class="navbar-text text-light opacity-75">''' + f"{stats['total_species']:,}" + ''' species | Updated ''' + build_date + '''</span>
        </div>
    </nav>
    <div class="container-fluid px-4">
        <div class="row">
            ...
            <div class="col-lg-9">
                <div class="row mb-4">
                    <div class="col-md-4">
                        <div class="chart-container"><h6 class="text-muted text-uppercase small mb-2">Coverage Overview</h6><div id="pieChart" style="height:280px;"></div></div>
                    </div>
                    <div class="col-md-8">
                        <div class="chart-container"><h6 class="text-muted text-uppercase small mb-2">Gap Analysis by Order (Top 20)</h6><div id="barChart" style="height:280px;"></div></div>
                    </div>
                </div>
                <div class="table-container">
                    <div class="d-flex justify-content-between align-items-center mb-3">
                    ...
                    <div id="loadingIndicator" class="loading"><div class="loading-spinner"></div><p class="mt-3 text-muted">Loading data...</p></div>
                    <div id="tableWrapper" style="display:none;">
                        <div class="table-responsive">
                            <table id="dataTable" class="table table-striped table-hover" style="width:100%"><thead></thead><tbody></tbody></table>
                        </div>
                    </div>
                </div>
            </div>
...
'''
    ...
    # JavaScript with embedded config
    js_config = f'''
    <script>
        const DATA_FILE = "{gene_name}.json.gz";
        const FILTER_OPTIONS = {json.dumps(filter_options)};
        const JNCC_COLUMNS = {json.dumps(jncc_info)};
        const STATUS_COLORS = {json.dumps(STATUS_COLORS)};
        const TOTAL_SPECIES = {stats['total_species']};
    ...
    js_main = '''
        ...
        function initializeTable() {
            const columns = [
                ...
                { data: 'order', title: 'Order' },
                { data: 'family', title: 'Family' },
                { data: 'species_status', title: 'Status', render: function(data) {
                    if (!data) return '';
        ...
        function applyFilters() {
            ...
            filteredData = DATA.filter(row => {
                ...
                if (filters.habitats.length > 0) {
                    const hasHabitat = (
                        (filters.habitats.includes('marine') && row.marine_flag === 'Y') ||
                        (filters.habitats.includes('freshwater') && row.freshwater === 'Y') ||
                        (filters.habitats.includes('terrestrial') && row.terrestrial_freshwater_flag === 'Y')
                    );
                    if (!hasHabitat) return false;
                }
                if (filters.protected) {
                    const hasProtection = JNCC_COLUMNS.some(col => row[col.original] && row[col.original] !== '');
        ...
        function updateActiveFiltersDisplay(filters) {
            const container = document.getElementById('activeFilters');
            container.innerHTML = '';
            const addTag = (label, value, clearFn) => {
                const tag = document.createElement('div');
                tag.className = 'filter-tag';
                tag.innerHTML = label + ': ' + value + ' <button onclick="' + clearFn + '">x</button>';
        ...
        function updateCharts() {
            ...
            filteredData.forEach(row => {
                const status = row.species_status || 'Unknown';
            ...
            Plotly.newPlot('pieChart', [{
                ...
                type: 'pie',
                hole: 0.4,
                marker: { colors: Object.keys(statusCounts).map(s => STATUS_COLORS[s] || '#6c757d') },
                textinfo: 'label+percent',
                textposition: 'outside'
            }], { margin: { t: 20, b: 20, l: 20, r: 20 }, showlegend: false }, { responsive: true });
            
            const orderData = {};
            filteredData.forEach(row => {
                const order = row.order || 'Unknown';
                const status = row.species_status || 'Unknown';
                if (!orderData[order]) orderData[order] = {};
                orderData[order][status] = (orderData[order][status] || 0) + 1;
            });
            
            const sortedOrders = Object.entries(orderData)
                .map(([order, counts]) => ({ order, total: Object.values(counts).reduce((a, b) => a + b, 0), counts }))
                .sort((a, b) => b.total - a.total)
                .slice(0, 20);
            
            const barTraces = ['GREEN', 'AMBER', 'RED', 'BLUE', 'BLACK'].map(status => ({
                x: sortedOrders.map(o => o.order),
                y: sortedOrders.map(o => o.counts[status] || 0),
                name: status,
                type: 'bar',
        ...
        function loadFiltersFromUrl() {
            ...
            if (params.get('habitat')) {
                params.get('habitat').split(',').forEach(h => {
                    const cb = document.getElementById('habitat' + h.charAt(0).toUpperCase() + h.slice(1));
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.getElementById('shareUrl').addEventListener('click', () => {
                navigator.clipboard.writeText(window.location.href).then(() => {
                    const indicator = document.getElementById('urlIndicator');
                    indicator.classList.add('show');
                    setTimeout(() => indicator.classList.remove('show'), 2000);
                });
            });
            document.getElementById('downloadFiltered').addEventListener('click', () => downloadCSV(filteredData, 'filtered_gap_analysis.csv'));
            document.getElementById('downloadAll').addEventListener('click', () => downloadCSV(DATA, 'full_gap_analysis.csv'));
        });
    </script>
</body>
</html>
'''
    ...
    output_path = Path(output_dir) / f"{gene_name}.html"
    with open(output_path, 'w', encoding='utf-8') as f:
        f.write(html_part1 + js_config + js_main)
...
def main():
    parser = argparse.ArgumentParser(description='Build UKBOL Gap Analysis website')
    parser.add_argument('--data', default='data', help='Path to data directory containing TSV files')
    parser.add_argument('--output', default='docs', help='Output directory for generated HTML')
    parser.add_argument('--zenodo', default=None, help='Path to zenodo_links.csv (optional)')
    args = parser.parse_args()
    ...
    for tsv_path in sorted(tsv_files):
        ...
        json_path = output_dir / f"{gene_name}.json.gz"
        num_records = prepare_data_json(df, jncc_columns, json_path)
        json_size = json_path.stat().st_size / (1024 * 1024)
        print(f"  Data file: {json_path.name} ({json_size:.1f} MB)")
